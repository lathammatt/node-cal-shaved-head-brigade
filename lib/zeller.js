...
const daysOfWeek = () => {
	let daysTitle = "Su Mo Tu We Th Fr Sa"
	console.log(daysTitle);
	return daysTitle
}
...
const monthSlice = (year, month, q) => {
	...
	let weekOne = finalArray.slice(0,7).join('')
	let weekTwo = finalArray.slice(7,14).join('')
	let weekThree = finalArray.slice(14,21).join('')
	let weekFour = finalArray.slice(21,28).join('')
	let weekFive = finalArray.slice(28,35).join('')
	let weekSix = finalArray.slice(35,42).join('')
	console.log(weekOne);
	console.log(weekTwo);
...
// console.log("check", monthSlice(2016, 5));




module.exports = {getMonth, getYear, getDayOfWeek, monthName, weekStart, monthCalc, yearCalc, monthDays, monthBuild, monthSlice, daysOfWeek}
